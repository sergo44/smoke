...
read -p "Please, enter version of PHP 7.1 to build (etc 7.1.1):" VER
...
if [ $REPLY = "y" ]; then
    ...
    echo "pid = /run/php/php-7.1-fpm.pid" >> $CFG/php-fpm.conf
    echo "error_log = /var/log/php-7.1-fpm.log" >> $CFG/php-fpm.conf
    echo "include=/opt/php-7.1-fpm.d/*.conf" >> $CFG/php-fpm.conf
...
#
# Set as default PHP-7.1 OPT CGI ?
#
read -p "Set as default version for php-7.1 opt?" REPLY
if [ $REPLY = "y" ]; then
    echo "#!/opt/php-$VER/bin/php-cgi" > /opt/php-cgi-7.1
    chmod 755 /opt/php-cgi-7.1

    rm /opt/php-7.1
    ln -s /opt/php-$VER /opt/php-7.1
fi;
