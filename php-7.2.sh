...
read -p "Please, enter version of PHP 7.2 to build (etc 7.2.0):" VER
...
if [ $REPLY = "y" ]; then
    ...
    echo "pid = /run/php/php-7.2-fpm.pid" >> $CFG/php-fpm.conf
    echo "error_log = /var/log/php-7.2-fpm.log" >> $CFG/php-fpm.conf
    echo "include=/opt/php-7.2-fpm.d/*.conf" >> $CFG/php-fpm.conf
...
#
# Set as default PHP-7.2 OPT CGI ?
#
read -p "Set as default version for php-7.2 opt?" REPLY
if [ $REPLY = "y" ]; then
    echo "#!/opt/php-$VER/bin/php-cgi" > /opt/php-cgi-7.2
    chmod 755 /opt/php-cgi-7.2

    rm /opt/php-7.2
    ln -s /opt/php-$VER /opt/php-7.2
fi;
